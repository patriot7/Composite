...
extern td_t ccv_ed_tsplit(spdid_t spdid, td_t td, char *param, int len, tor_flags_t tflags, long evtid);
extern int ccv_ed_twritep(spdid_t spdid, td_t td, int cbid, int sz);
extern int ccv_ed_treadp(spdid_t spdid, td_t td, int *off, int *len);
...
cos_init(void)
{
        ...
        td = ccv_ed_tsplit(cos_spd_id(), td_root, "3", strlen("3"), TOR_ALL, evt_tsplit);
        ...
        printc("begin to twritep\n");
        ccv_ed_twritep(cos_spd_id(), td, cbuf_mat->cbid, cbuf_mat->size);
        printc("twritep done\n");

        /* test of treapd, should same with the input*/ 
        int off, len;

        cbid = ccv_ed_treadp(cos_spd_id(), td, &off, &len);
